...
class WordleBoardViewModel: ObservableObject {
    ...
    init(height: Int = 3) {
        if let currentClue = DAO.instance?.currentClue
        {
            self.clue = currentClue
        } else {
            self.clue = Clue(nextLocation: "SAMPLE", location: "SAMPLE")
        }
        self.width = self.clue.solution.count
        self.height = height
        letters = Array(
            repeating: .init(repeating: nil, count: self.clue.solution.count),
            count: height
        )
        evaluations = Array(
